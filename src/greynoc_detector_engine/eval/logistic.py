...
Adapted from the GreyNOC GN-SLOP-DETECTION ``app/eval/logistic.py``. Two places
in the harness need to fit a logistic model and neither justifies pulling in
scikit-learn (which would violate the project's no-mandatory-heavy-dependency
rule and ship a 30 MB wheel for ~60 lines of math):

  * **Platt scaling** (``platt.py``) -- one feature (a raw forecast score),
    mapping it to a calibrated probability.
  * **Glass-box fusion weights** (``runner.learn_fusion_weights``) -- many
    features (the per-driver contributions of the forecaster), learning
    interpretable coefficients that drop into ``scoring.yaml``.
...
def fit_logistic(
    ...
    *,
    ...
    standardized = [[(row[j] - means[j]) / stds[j] for j in range(d)] for row in features]

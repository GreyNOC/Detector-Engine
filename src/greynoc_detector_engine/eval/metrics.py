...
Adapted from the GreyNOC GN-SLOP-DETECTION ``app/eval/metrics.py`` harness.
Everything here operates on the standard convention used throughout this
package:

    label 1 == the POSITIVE class == "threat was actually exploited"
    label 0 == the NEGATIVE class == "no exploitation observed"
    score   == a real number where HIGHER means MORE likely to be attacked
               (e.g. ``AttackForecast.attack_probability``).

The headline numbers are deliberately chosen to match how the detection /
forecasting literature reports results:

  * **ROC-AUC** -- threshold-free separability.
  * **TPR @ fixed FPR** (1%, 5%, 10%) -- the number that actually matters for a
    triage queue. A forecaster that flags 95% of exploited CVEs while also
    flagging 30% of the inert ones drowns the SOC. Reporting TPR at a pinned
    low FPR forces the false-positive cost into the open.
...
def roc_curve(scores: Sequence[Number], labels: Sequence[int]) -> list[RocPoint]:
    ...
    is anchored at (0,0) so interpolation is always well-defined.
    """
    _validate(scores, labels)
    n_pos = sum(1 for label in labels if label == 1)
    n_neg = len(labels) - n_pos
...
def tpr_at_fpr(scores: Sequence[Number], labels: Sequence[int], target_fpr: float) -> float | None:
    ...
    This is the operating-point question a triage owner actually asks: "if I will
    tolerate chasing at most 5% of inert CVEs, how many real attacks do I catch
    early?" Returns ``None`` if the corpus has only one class.
    """
    ...
    for point in curve:
        ...
        # we neither over- nor under-credit the forecaster at the exact budget.
...
def best_f1_threshold(scores: Sequence[Number], labels: Sequence[int]) -> ThresholdMetrics | None:
    ...
    (fewer positives), which is the conservative choice for a triage queue.
    """
    _validate(scores, labels)
    if not scores:
        return None
...
def expected_calibration_error(
    ...
    a caller never reads ECE off a raw, unbounded score by mistake.
    """
    _validate(scores, labels)
...
def brier_score(scores: Sequence[Number], labels: Sequence[int]) -> float | None:
    ...
    if any(score < 0.0 or score > 1.0 for score in scores):
        return None
    return sum((float(s) - label) ** 2 for s, label in zip(scores, labels, strict=True)) / len(
        scores
    )
...
class EvaluationReport:
    """The full metric picture for one forecaster over one corpus."""
    ...
    def as_dict(self) -> dict[str, object]:
        return {
            ...
            "tpr_at_1pct_fpr": (
                None if self.tpr_at_1pct_fpr is None else round(self.tpr_at_1pct_fpr, 4)
            ),
            "tpr_at_5pct_fpr": (
                None if self.tpr_at_5pct_fpr is None else round(self.tpr_at_5pct_fpr, 4)
            ),
            "tpr_at_10pct_fpr": (
                None if self.tpr_at_10pct_fpr is None else round(self.tpr_at_10pct_fpr, 4)
            ),

...
from greynoc_detector_engine.catalog.deduplication import (
    merge_threats,
...
class ThreatLibrary:
    ...
    def create(self, threat: ThreatRecord) -> ThreatRecord:
        self.storage.upsert_threat(threat)
        return threat

    def update(self, threat: ThreatRecord, reason: str) -> ThreatRecord:
        updated = threat.model_copy(deep=True)
        updated.version += 1
        updated.changelog.append(reason)
        self.storage.upsert_threat(updated)
        return updated

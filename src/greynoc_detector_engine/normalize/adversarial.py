"""Adversarial-evasion detection and normalization for ingested OSINT text.

Adapted from the GreyNOC GN-SLOP-DETECTION ``app/core/adversarial.py`` and
retargeted at threat-intel ingestion. A lexicon/regex entity extractor (CVE
ids, product names, actor aliases, AI-attack terms) is trivially blinded by
character-level tricks that leave the text looking identical to a human reader:

  * **Zero-width / invisible characters** (U+200B..U+200D, U+FEFF, U+2060,
    soft hyphen) spliced inside words -- ``Fortinet`` with a zero-width space in
    the middle no longer matches the product regex, and the tokenizer splits it.
  * **Homoglyph substitution** -- Cyrillic ``о``/``е``/``а`` or Greek ``ο``/``ν``
    swapped for Latin lookalikes so a flagged token evades an exact match while
    looking unchanged. NFKC does NOT fold these (distinct letters in distinct
    scripts), so ordinary normalization misses them.
...
detection:
  ...
  1. ``scan_evasion`` -- *report* the obfuscation as a finding in its own right.
     Mixed-script tokens and bidi controls are near-zero-false-positive tells of
     deliberate tampering; their presence is itself signal worth surfacing to an
     analyst (a feed quietly injecting bidi controls is suspect).
  2. ``normalize_adversarial`` -- *defeat* the obfuscation by stripping
     ...
     regularizing whitespace, so downstream entity extraction still fires on the
...
can silently delete them. The homoglyph table is a curated set of high-frequency
attack characters, not the full Unicode confusables database -- enough to defeat
the common evasions while staying small and auditable.
"""
...
class EvasionReport:
    ...
    def as_dict(self) -> dict[str, object]:
        return {
            ...
            "exotic_spaces": self.exotic_spaces,
            "confusable_chars": self.confusable_chars,
            "mixed_script_words": self.mixed_script_words,
            "is_evasive": self.is_evasive,
...
def normalize_adversarial(text: str) -> str:
    ...
    Returns text that is semantically what a human reader sees, so downstream
    entity extraction cannot be evaded by character-level tricks. Does not
    collapse runs of whitespace -- that is left to the engine's existing
    ``normalize_whitespace`` so behavior on clean text is unchanged.
    """
    if not text:
        return text
    out: list[str] = []

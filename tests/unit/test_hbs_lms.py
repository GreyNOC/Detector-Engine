"""Tests for the pure-Python RFC 8554 LMS / HSS backend.

The known-answer test parses the *real* Test Case 1 vectors from RFC 8554
Appendix F (HSS, two levels, LMS_SHA256_M32_H5 / LMOTS_SHA256_N32_W8). If that
vector verifies, the wire format, domain separators, Winternitz chains, checksum
and Merkle climb are all byte-for-byte correct.
"""
...
from greynoc_detector_engine.crypto.hbs import (
    LMOTS_SHA256_N32_W8,
    LMS_SHA256_M32_H5,
    HssPrivateKey,
    HssPublicKey,
...
# --- RFC 8554 Appendix F, Test Case 1 (HSS, L=2) --------------------------

# HSS public key = u32(levels=2) || LMS public key of the top tree.
TC1_PUBLIC_KEY = bytes.fromhex(
    "00000002"
    "00000005"  # LMS_SHA256_M32_H5
    "00000004"  # LMOTS_SHA256_N32_W8
    "61a5d57d37f5e46bfb7520806b07a1b8"  # I
    "50650e3b31fe4a773ea29a07f09cf2ea"  # T[1] (high)
    "30e579f0df58ef8e298da0434cb2b878"  # T[1] (low)
)

# "The powers not delegated to the United States by the Constitution, nor
#  prohibited by it to the States, are reserved to the States respectively,
#  or to the people.\n"  -- note the trailing 0x2e0a.
TC1_MESSAGE = bytes.fromhex(
...
TC1_SIGNATURE = bytes.fromhex(
    "00000001"  # Nspk
    # sig[0]: top-level LMS signature
    "00000005"  # q
    "00000004"  # LMOTS type
    "d32b56671d7eb98833c49b433c272586"  # C
    ...
    # pub[1]: bottom-tree LMS public key
    "00000005"  # LMS type
    "00000004"  # LMOTS type
    "d2f14ff6346af964569f7d6cb880a1b6"  # I
    "6c5004917da6eafe4d9ef6c6407b3db0"  # T[1]
    "e5485b122d9ebe15cda93cfec582d7ab"
    # sig[1]: bottom-level LMS signature (final_signature)
    "0000000a"  # q
    "00000004"  # LMOTS type
    "0703c491e7558b35011ece3592eaa5da"  # C
...
def test_rfc8554_test_case_1_kat() -> None:
    assert hss_verify(TC1_PUBLIC_KEY, TC1_MESSAGE, TC1_SIGNATURE) is True


def test_rfc8554_kat_rejects_flipped_signature_byte() -> None:
    tampered = bytearray(TC1_SIGNATURE)
    tampered[100] ^= 0x01
    assert hss_verify(TC1_PUBLIC_KEY, TC1_MESSAGE, bytes(tampered)) is False


def test_rfc8554_kat_rejects_flipped_message_byte() -> None:
    tampered = bytearray(TC1_MESSAGE)
    tampered[0] ^= 0x01
    assert hss_verify(TC1_PUBLIC_KEY, bytes(tampered), TC1_SIGNATURE) is False


def test_rfc8554_kat_via_public_key_object() -> None:
    pub = HssPublicKey.from_bytes(TC1_PUBLIC_KEY)
    assert pub.verify(TC1_MESSAGE, TC1_SIGNATURE) is True
    assert pub.algorithm.startswith("HSS-L2")


def test_lms_roundtrip_three_messages() -> None:
    key = generate_lms_key(lms_type=LMS_SHA256_M32_H5, lmots_type=LMOTS_SHA256_N32_W8)
    pub = key.public_key()
    pub_bytes = pub.to_bytes()
    messages = [b"alpha", b"bravo", b"charlie"]
    for message in messages:
        signature = key.sign(message)
        assert pub.verify(message, signature) is True
        assert lms_verify(pub_bytes, message, signature) is True
        # A different message must not verify under this signature.
        assert lms_verify(pub_bytes, message + b"!", signature) is False


def test_lms_public_key_wire_roundtrip() -> None:
    key = generate_lms_key(lms_type=LMS_SHA256_M32_H5)
    pub = key.public_key()
    restored = LmsPublicKey.from_bytes(pub.to_bytes())
    assert restored.to_bytes() == pub.to_bytes()
    assert restored.algorithm == pub.algorithm
    assert len(pub.to_bytes()) == 56


def test_lms_state_advances_and_decrements() -> None:
    key = generate_lms_key(lms_type=LMS_SHA256_M32_H5)
    start = key.remaining()
    assert start == 2**5
    sig_a = key.sign(b"first")
    assert key.remaining() == start - 1
    sig_b = key.sign(b"second")
    assert key.remaining() == start - 2
    # Distinct leaves => the leaf index q encoded in the first 4 bytes differs.
    assert sig_a[:4] != sig_b[:4]


def test_lms_exhaustion_raises() -> None:
    # A height-5 tree has 32 leaves; deterministic seed keeps keygen cheap.
    key = generate_lms_key(
        lms_type=LMS_SHA256_M32_H5,
        identifier=b"\x01" * 16,
        seed=b"\x02" * 32,
    )
    for i in range(2**5):
        key.sign(f"m{i}".encode())
    assert key.remaining() == 0
    with pytest.raises(RuntimeError, match="exhausted"):
        key.sign(b"one too many")


def test_lms_serialization_preserves_state() -> None:
    key = generate_lms_key(lms_type=LMS_SHA256_M32_H5)
    pub_bytes = key.public_key().to_bytes()
    key.sign(b"used-leaf-0")
    key.sign(b"used-leaf-1")
    state = key.to_dict()
    assert state["q"] == 2

    restored = LmsPrivateKey.from_dict(state)
    # Same public key (same I and SEED) and the counter survived the round trip.
    assert restored.public_key().to_bytes() == pub_bytes
    assert restored.remaining() == key.remaining()

    signature = restored.sign(b"after-restore")
    assert signature[:4] == (2).to_bytes(4, "big")  # next leaf is q=2
    assert lms_verify(pub_bytes, b"after-restore", signature) is True


def test_hss_two_level_sign_verify_and_tamper() -> None:
    key = generate_hss_key(levels=2, lms_type=LMS_SHA256_M32_H5)
    pub = key.public_key()
    pub_bytes = pub.to_bytes()
    message = b"hierarchical signature payload"
    signature = key.sign(message)
    assert hss_verify(pub_bytes, message, signature) is True
    assert pub.verify(message, signature) is True

    tampered = bytearray(signature)
    tampered[-1] ^= 0x01
    assert hss_verify(pub_bytes, message, bytes(tampered)) is False
    assert hss_verify(pub_bytes, message + b"x", signature) is False


def test_hss_single_level_round_trip() -> None:
    key = generate_hss_key(levels=1, lms_type=LMS_SHA256_M32_H5)
    pub_bytes = key.public_key().to_bytes()
    signature = key.sign(b"flat hss")
    assert signature[:4] == (0).to_bytes(4, "big")  # Nspk == 0
    assert hss_verify(pub_bytes, b"flat hss", signature) is True


def test_hss_serialization_round_trip() -> None:
    key = generate_hss_key(levels=2, lms_type=LMS_SHA256_M32_H5)
    pub_bytes = key.public_key().to_bytes()
    key.sign(b"before")
    restored = HssPrivateKey.from_dict(key.to_dict())
    assert restored.public_key().to_bytes() == pub_bytes
    signature = restored.sign(b"after-restore")
    assert hss_verify(pub_bytes, b"after-restore", signature) is True
